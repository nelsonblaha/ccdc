...
flowchart TB
    ...
    subgraph site["chucodata-site — custom, small"]
    ...
    subgraph custom["chucodata-ledger — custom, small"]
...
### 1. `chucodata-site` — public website
...
### 2. `chucodata-ledger` — membership and credit service
...
2. asks `chucodata-ledger` for the balance
...
**Phase 1 — make it real (~3 weeks).** `chucodata-site` + Stripe + the pending→user
...
**Phase 2 — make it sustainable (~2 weeks).** `chucodata-ledger` + the inlet
